...
enum Commands {
    ...
    /// Insert a vector with optional payload. ID is auto-generated by the server.
    Insert {
        #[arg(short, long)]
            ...
            match cli.command {
                ...
                Commands::CollectionExists { collection } => {
                    let exists = client
                        ...
                        .map_err(|e| anyhow::anyhow!("{e}"))?;
                    if exists {
                        println!("Collection '{collection}' exists");
                    } else {
                        println!("Collection '{collection}' does not exist");
                    }
                }
                Commands::Insert { payload, collection } => {
...
mod tests {
    ...
    fn test_cli_parse_server() {
        ...
        match cli.command {
            Commands::Server { config } => assert_eq!(config, "config.yaml"),
    ...
    fn test_cli_parse_delete() {
        ...
        match cli.command {
            Commands::Delete { ids, .. } => assert_eq!(ids, vec![1, 2, 3]),
    ...
    fn test_cli_parse_create_collection() {
        ...
        match cli.command {
            Commands::CreateCollection { collection, .. } => assert_eq!(collection, "new_col"),
    ...
    fn test_cli_parse_drop_collection() {
        ...
        match cli.command {
            Commands::DropCollection { collection } => assert_eq!(collection, "old_col"),
    ...
    fn test_cli_parse_collection_exists() {
        ...
        match cli.command {
            Commands::CollectionExists { collection } => assert_eq!(collection, "test_col"),
