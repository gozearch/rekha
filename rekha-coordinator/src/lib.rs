use rekha_core::{
    ...
    VectorRecord, VectorStoreBackend, now_epoch_secs, now_micros,
};
...
fn parse_consistency(s: &str) -> Option<ConsistencyLevel> {
...
impl Coordinator {
    ...
    pub async fn drop_collection(
        ...
        let required = match consistency {
            ConsistencyLevel::One => 1usize, ConsistencyLevel::Quorum => drop_rf / 2 + 1, ConsistencyLevel::All => drop_rf,
        };
                ...
                match client.try_remote_drop_collection(name, timestamp).await {
                    ...
                    Err(_) => {
                        if self.config.hinted_handoff_enabled {
                            let _ = self.store.hint_store().put_collection_hint(node_id, name, &[], timestamp, 1);
                        }
                    }
                }
            }
    ...
    pub async fn create_collection(
        ...
        let required = match consistency {
            ConsistencyLevel::One => 1, ConsistencyLevel::Quorum => rf_val / 2 + 1, ConsistencyLevel::All => rf_val,
        };
        let mut acks = 1u64;
        for node_id in &peer_ids {
            let mut pool = self.peer_pool.write().await;
            if let Some(client) = pool.clients.get_mut(node_id) {
                match client.try_remote_create_collection(name, &proto_cfg, timestamp).await {
                    ...
                    Err(_) => {
                        if self.config.hinted_handoff_enabled {
                            let _ = self.store.hint_store().put_collection_hint(node_id, name, &[], timestamp, 0);
                        }
                    }
                }
            }
    ...
    pub async fn insert(
            ...
            for replica in replicas {
                ...
                if let Some(client) = pool.clients.get_mut(&replica.node_id) {
                    match client.try_remote_insert(collection, id, &vector, &pdata, timestamp).await {
                        ...
                        Err(_) => {
                            if self.config.hinted_handoff_enabled {
                                hints.push((replica.node_id.clone(), collection.to_string(), id, vector.clone(), pdata.clone(), timestamp));
                            }
                        }
                    }
                } else if self.config.hinted_handoff_enabled {
                    hints.push((replica.node_id.clone(), collection.to_string(), id, vector.clone(), pdata.clone(), timestamp));
                }
            }
        }
        ...
        for (node_id, col, hid, hvec, hpayload, hts) in &hints {
            let _ = self.store.hint_store().put_hint(node_id, col, *hid, hvec, hpayload.as_deref(), *hts);
        }

        let rf = self.read_collection_config(collection).map(|c| c.replication_factor as usize).unwrap_or(1);
        let required = match consistency {
            ConsistencyLevel::One => 1, ConsistencyLevel::Quorum => rf / 2 + 1, ConsistencyLevel::All => rf,
        };
                ...
                for replica in replicas {
                    ...
                    if let Some(client) = pool.clients.get_mut(&replica.node_id) {
                        match client.try_remote_delete(collection, &[*id], timestamp).await {
                            Ok(_) => acks += 1,
                            Err(_) => { if self.config.hinted_handoff_enabled { hints.push((replica.node_id.clone(), *id, timestamp)); } }
                        }
                    } else if self.config.hinted_handoff_enabled { hints.push((replica.node_id.clone(), *id, timestamp)); }
                }
            }
        }
    ...
    pub async fn peer_address(&self, node_id: &str) -> Option<String> {
        self.peers.read().await.get(node_id).map(|p| p.info.address.clone())
    }
    ...
    pub async fn register_peer(&self, info: NodeInfo) {
        ...
        self.peers.write().await.insert(node_id.clone(), PeerState { info: info.clone(), last_seen: Instant::now() });
        self.partition_manager.write().await.register_node(info);
    ...
    pub async fn check_peer_health(&self) {
        ...
        for peer in peers.values_mut() {
            if peer.last_seen.elapsed() > PEER_TIMEOUT && peer.info.status == NodeStatus::Healthy {
                peer.info.status = NodeStatus::Unreachable; changed = true;
            } else if peer.last_seen.elapsed() <= PEER_TIMEOUT && peer.info.status == NodeStatus::Unreachable {
                peer.info.status = NodeStatus::Healthy; changed = true;
                recovered_nodes.push(peer.info.node_id.clone());
            }
        }
        drop(peers);
        if changed { self.sync_topology().await; }

        if !recovered_nodes.is_empty() && self.config.hinted_handoff_enabled {
    ...
    async fn sync_topology(&self) {
        ...
        for peer in peers.values() { nodes.insert(peer.info.node_id.clone(), peer.info.clone()); }
...
mod tests {
    ...
    fn temp_store() -> Arc<RocksVectorStore> {
        ...
        let _p = dir2.into_path();
        Arc::new(rekha_storage::RocksVectorStore::open(&_p).unwrap())
    }
    ...
    fn test_coordinator() -> Coordinator {
        ...
        let pm = Arc::new(RwLock::new(rekha_partition::PartitionManager::new(HashMap::new(), 4, 768)));
    ...
    async fn test_create_collection_duplicate_returns_false() {
        ...
        let index = rekha_index::RekhaIndex::new().unwrap();
        coord.initialize(index).await;
        assert!(coord.create_collection("dup_test", 8, 16, 4, 1, 100, ConsistencyLevel::One).await.unwrap());
        assert!(!coord.create_collection("dup_test", 8, 16, 4, 1, 100, ConsistencyLevel::One).await.unwrap());
        assert!(coord.create_collection("dup_test", 8, 16, 4, 1, 200, ConsistencyLevel::One).await.unwrap());
    }
