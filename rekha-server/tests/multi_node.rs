...
async fn start_server(name: &str, seed_addrs: &[String]) -> TestServer {
    ...
    let coordinator = Arc::new(Coordinator::new(
        store, membership, node_id, name.to_string(),
        false, 3600, ConsistencyLevel::Quorum, 3,
        chord, peer_pool, 86400,
    ));
...
async fn add_successor(coordinator: &Coordinator, name: &str, addr: &str) {
    ...
    if !sl.contains(&name.to_string()) {
        ...
        coordinator.chord.successor_addresses.write().await.push(addr.to_string());
    }
}
...
async fn test_two_node_replication() {
    ...
    let chord2 = Arc::new(rekha_cluster::chord::ChordNode::new(chord_id2, "127.0.0.1:0"));
    let peer_pool2 = Arc::new(PeerPool::new());
    let coord2 = Coordinator::new(
        store2, membership2, 2, "node2".to_string(),
        false, 3600, ConsistencyLevel::Quorum, 3,
        chord2.clone(), peer_pool2, 86400,
    );
    ...
    coord2.create_collection("test", ivf_config(4), "node2", 1000, ConsistencyLevel::Quorum, false).await.unwrap();

    let mut client1 = rekha_client::Client::connect(&addr1).await.unwrap();
    let colls = client1.list_collections().await.unwrap();
    assert!(colls.contains(&"test".to_string()), "collection should be replicated to server1");
    ...
    assert!(!results.is_empty(), "replicated data should be visible on server1");

    let results2 = coord2.search("test", vec![0.1, 0.2, 0.3, 0.4], 5, SearchParams::default()).await.unwrap();
    assert!(!results2.is_empty(), "data should be visible on coordinator 2");
...
async fn test_collection_replicated_across_nodes() {
    ...
    let chord2 = Arc::new(rekha_cluster::chord::ChordNode::new(
        hash_to_chord_id(b"node2"), "127.0.0.1:0",
    ));
    let peer_pool2 = Arc::new(PeerPool::new());
    let coord2 = Coordinator::new(
        store2, membership2, 2, "node2".to_string(),
    ...
    coord2.create_collection("crosscol", ivf_config(4), "node2", 1000, ConsistencyLevel::Quorum, false).await.unwrap();

    let mut client1 = rekha_client::Client::connect(&addr1).await.unwrap();
    let names = client1.list_collections().await.unwrap();
    assert!(names.contains(&"crosscol".to_string()), "collection should be replicated to server1");
}
...
async fn test_insert_replicated_across_nodes() {
    ...
    chord2.successor_list.write().await.push("node1".to_string());
    chord2.successor_addresses.write().await.push(addr1_val.clone());

    coord2.create_collection("test", ivf_config(4), "node2", 1000, ConsistencyLevel::Quorum, false).await.unwrap();
    coord2.insert("test", 42, vec![0.5, 0.6, 0.7, 0.8], None, 1000, "node2", ConsistencyLevel::One, false).await.unwrap();

    let mut client1 = rekha_client::Client::connect(&addr1).await.unwrap();
    let results = client1.search("test", vec![0.5, 0.6, 0.7, 0.8], 5, SearchParams::default()).await.unwrap();
    assert!(!results.is_empty(), "replicated insert should be searchable on server1");
...
async fn test_delete_replicated_across_nodes() {
    ...
    client1.insert("test", 1, vec![0.1, 0.2, 0.3, 0.4], None, 1000, ConsistencyLevel::One).await.unwrap();
    ...
    coord2.delete("test", &[1], 1001, "node2", ConsistencyLevel::One, false).await.unwrap();

    let results = client1.search("test", vec![0.1, 0.2, 0.3, 0.4], 5, SearchParams::default()).await.unwrap();
    assert!(results.is_empty(), "deleted vector should not be on server1 after replicated delete");
}
...
async fn test_search_fanout_merges_from_remote() {
    ...
    coord2.create_collection("test", ivf_config(4), "node2", 1000, ConsistencyLevel::Quorum, false).await.unwrap();
    coord2.insert("test", 10, vec![0.1, 0.2, 0.3, 0.4], None, 1000, "node2", ConsistencyLevel::One, false).await.unwrap();

    let mut client1 = rekha_client::Client::connect(&addr1).await.unwrap();
    client1.insert("test", 20, vec![0.9, 0.8, 0.7, 0.6], None, 1000, ConsistencyLevel::One).await.unwrap();

    let results = client1.search("test", vec![0.1, 0.2, 0.3, 0.4], 5, SearchParams {
        local_only: false,
        ..SearchParams::default()
    }).await.unwrap();
    assert!(!results.is_empty(), "fan-out search should return results");
    assert_eq!(results[0].id, 10, "nearest vector should be the exact match from coord2's replica");
}
...
async fn test_3_node_replication() {
    ...
    c1.insert("images", 1, vec![0.1; 8], None, 1000, ConsistencyLevel::One).await.unwrap();

    // Search from all 3 nodes — each should find the data
    for (name, addr) in [("node1", &a1), ("node2", &a2), ("node3", &a3)] {
        let mut client = rekha_client::Client::connect(addr).await.unwrap();
        let results = client.search("images", vec![0.1; 8], 5, SearchParams::default()).await.unwrap();
...
async fn test_failover_after_node_stop() {
    ...
    let results = c2.search("images", vec![0.1; 8], 5, SearchParams::default()).await.unwrap();
    assert!(!results.is_empty(), "search should work after node-1 failure");
}
...
async fn test_node_restart_serves_data() {
    ...
    let results = c1r.search("images", vec![0.1; 8], 5, SearchParams::default()).await.unwrap();
...
async fn test_multi_collection_cross_node() {
    ...
    c2.insert("texts", 1, vec![0.5; 4], None, 1000, ConsistencyLevel::One).await.unwrap();

    // Search 'images' from node-3
    let mut c3 = rekha_client::Client::connect(&a3).await.unwrap();
    let r_images = c3.search("images", vec![0.1; 8], 5, SearchParams::default()).await.unwrap();
    assert!(!r_images.is_empty(), "images collection searchable from node-3");

    // Search 'texts' from node-3
    let r_texts = c3.search("texts", vec![0.5; 4], 5, SearchParams::default()).await.unwrap();
    assert!(!r_texts.is_empty(), "texts collection searchable from node-3");
}
